...
Every step from a fresh clone to a measured error for every simplification. Commands are
PowerShell where they differ; run them from the repository root.

Each step says roughly what it costs, because two of them are measured in hours.
...
On Windows PowerShell 5.1, which has no `-Parallel`, use `Start-Job` instead. Why the shards
stride rather than take blocks, and where that still goes wrong, is in
...
writes files and needs no display.
...
has one. This also fills in the statistics panel's Fréchet line:

```python
import json, pathlib

name, algo, rate = "mopsi-000003.json", "dots", "m3"
ds = "mopsi"
